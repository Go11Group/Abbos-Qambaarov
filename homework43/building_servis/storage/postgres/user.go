...
func (s *UserRepo) CreateUser(user *models.CreateUser) error {

	_, err := s.Db.Exec("insert into users(id, name, age, phone) values ($1, $2)",
		uuid.NewString(), user.Name, user.Age, user.Phone)
...
func (s *UserRepo) UpdateUser(user models.CreateUser ,id string) error {
