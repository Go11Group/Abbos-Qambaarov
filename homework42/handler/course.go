...
// handlerdan course Lesson API ni ishlatish
func (co *Handler1) CourseLessons(c *gin.Context) {
	//urldan idni olish
	id := c.Param("course_id")
	course, err := co.PostCourse.GetCourseLessons(id)
	if err != nil {
	 err = fmt.Errorf("error server: %s", err)
...
// handlerdan course users API ni ishlatish
func (co *Handler1) CourseUsers(c *gin.Context) {
	//urldan idni olish
	id := c.Param("course_id")
	course, err := co.PostCourse.GetCourseUsers(id)
	if err != nil {
	 err = fmt.Errorf("error server: %s", err)
	 c.JSON(http.StatusBadRequest, err.Error())
	} else {
